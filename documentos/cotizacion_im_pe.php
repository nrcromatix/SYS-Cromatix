...
<td align="right"><b><?php echo "$".number_format("$monto_unitario_venta",$d,",","."); ?></b></td>
...
<td align="right"><b><?php echo "$".number_format("$monto_total_venta_dcto",$d,",",".");?></b></td>
</tr>
...
<tr>
 <th colspan="8" align="right">Descuento Global Neto:</th><td colspan="3" align="right"><font size="2"><?php echo "(".$descuento_p." %) - $ ".number_format("$monto_descuento_neto",$d,",",".");?></font></td>

</tr>
<?php } ?>
<tr>
 <th <?php if ($dcto_total_items>0) { echo 'colspan="9"'; } else { echo 'colspan="8"'; } ?> align="right">Total Neto:</th><td colspan="3" align="right"><font size="2"><?php echo "$ ".number_format("$total_neto",$d,",",".");?></font></td>

</tr>
<?php if ($tipo<>"FACTURA EXENTA" and $tipo<>"QUOTATION") { ?>
<tr>
 <th <?php if ($dcto_total_items>0) { echo 'colspan="9"'; } else { echo 'colspan="8"'; } ?> align="right">I.V.A.:</th><td colspan="3" align="right"><font size="2"><?php echo "$ ".number_format("$iva",$d,",",".");?></font></td>

</tr>

<tr>
 <th <?php if ($dcto_total_items>0) { echo 'colspan="9"'; } else { echo 'colspan="8"'; } ?> align="right">Total Venta Actual:</th><td colspan="3" align="right"><font size="2"><b><?php echo "$ ".number_format("$total_venta",$d,",",".");?></b></font></td>
</tr>
<?php } ?>
<!--
<tr>
...
<?php echo $observaciones; ?>
  ...
  <h2>* Cotizaci&oacute;n v&aacute;lida por 5 d&iacute;as corridos a partir de fecha de emisi&oacute;n.</h2>

<table class="tablacot" border="1" width="600">
	<tr><th><center>
Datos Cuenta Bancaria: CROMATIX SPA - RUT: 76.836.147-9 - CTA-CTE SANTANDER: 73046467<br> MAIL: INFO@CROMATIX.CL - WWW.CROMATIX.CL</center>
	</th>
	</tr>
	...
	<center><b>CONSIDERACIONES ACEPTACION DE PROPUESTA</b></center>	
Garantía de los equipos es de 1 año, según póliza de garantía, lo que aplica al equipamiento y de considerarla los productos se deben entregar en nuestras dependencias, la cual no considera problemas debido a la instalación eléctrica o por
mala manipulación de los mismos. Estos serán revisados para determinar la procedencia de falla.<br>
Para productos de fabricación e importación los plazos de entrega son máximos estimativos a partir de confirmación con anticipo, CROMATIX no se hace responsable por retrasos correspondientes a razones externas
de fabricación e importación.<br>
Precio considera venta de equipos y no considera instalación de los mismos a excepción de que la cotización lo mencione explícitamente en un ítem.<br>
La entrega de los equipos se realizara contra pago de saldo de los productos en fecha previamente coordinada y según condiciones acordadas. Incluye despacho en Región Metropolitana por compras superiores a $500.000 CLP.<br>
Para LEDs strip y Perfiles, no se aceptan devoluciones una vez cortado o instalado el producto. Garantía cubre solo por defectos de fabricación, por lo cual debe ser probado antes de instalar, para así cobrar garantía
si hubiese falla de fábrica. El producto debe ser devuelto en su envase original completo. Para componentes eléctricos y LEDs strip deben quedar registrables e instalados según indica EE.TT respectivas.<br>
Entrega de productos a convenir. Despacho dentro de Santiago con previa coordinación y disponibilidad dentro de 2 a 5 días hábiles. La entrega se realiza sobre camión.<br>
CROMATIX no se hace responsable por cantidades o largos de perfiles cotizados, lo que es validado y entregado por el mandante o cliente al aprobar esta cotización.	<br>
Valores expresados en moneda nacional (CLP). CROMATIX se reserva el derecho a modificaciones de precios producidos por alza excesiva del dolar (USD) al momento de aceptación.
	</td>
	</tr>
