...
$cantidad_mod = $_POST["cantidad_mod"];
$tipo = $_GET["tipo"];
...
$monto_unitario_venta=$row_n["monto_unitario_venta"];	
...
$monto_total_venta=$monto_unitario_venta*$cantidad_mod;

$diff_neto=$monto_unitario_venta*($cantidad_mod-$cantidad);


//REBAJA DE INVENTARIO

if ($tipo=="BOLETA" OR $tipo=="GUIA") {	
	if ($tipo=="NOTA DE CREDITO") { $cantidad_dif=$cantidad_mod-$cantidad; } else { $cantidad_dif=-1*($cantidad_mod-$cantidad); } //MODIFICAR DESPUES PARA GUIA DE DEVOLUCION
...
$sql = "UPDATE detalle_documento SET cantidad='".$cantidad_mod."', monto_total_venta='".$monto_total_venta."' WHERE id_detalle='".$id_detalle."'";
if ($db->query($sql) === TRUE) {
	$sql_t = "UPDATE documentos SET total_venta_neto=total_venta_neto+'".$diff_neto."' WHERE cod_documento='".$cod_documento."'";
